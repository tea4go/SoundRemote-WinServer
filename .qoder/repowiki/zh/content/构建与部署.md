...
## 更新摘要
**所做更改**
- 增强了 run_win.ps1 脚本的依赖管理系统，包含 -InstallDeps 和 -Check 参数
- 改进了环境检测功能，支持 vswhere、Visual Studio C++ 工作负载、MSBuild、平台工具集 v145、Windows 10 SDK、vcpkg 可访问性、NuGet 可用性和包还原状态检查
- 添加了智能依赖安装逻辑，包括自动 nuget.exe 下载和 vcpkg 集成支持
- 更新了开发环境搭建指南，反映新的自动化功能
...
**更新** 增强了依赖管理系统的自动化能力，提供更智能的环境检测和一键式依赖安装功能。
...
- [run_win.ps1:1-283](file://run_win.ps1#L1-L283)
...
**更新** 增强了 run_win.ps1 脚本的智能依赖管理和环境检测功能。
...
- [run_win.ps1:167-202](file://run_win.ps1#L167-L202)
- [run_win.ps1:204-249](file://run_win.ps1#L204-L249)
- [run_win.ps1:251-283](file://run_win.ps1#L251-L283)
...
- **新增** 智能环境检测与修复
  ...
  - 自动下载缺失的 nuget.exe 到用户目录。
  - 智能检测 vcpkg 依赖是否已安装，避免重复触发 cmake 等工具链下载。
- 推荐步骤
...
- **增强** 常用参数
  - -Check：环境检测，显示详细的组件状态和修复建议
  - -InstallDeps：安装依赖（自动下载 nuget.exe + nuget restore + vcpkg install）
...
- **增强** 安装顺序与智能检测
  ...
  - 自动检测 vcpkg 依赖是否已安装，避免重复下载
  - 自动下载缺失的 nuget.exe 到用户目录
  - 支持 VS 内置 vcpkg 和独立安装的 vcpkg

**更新** 依赖管理系统现在具有更强的容错能力和智能化特性。
...
- **增强** 常见错误与修复
  ...
  - **新增** 依赖安装问题：使用 -Check 查看详细的状态报告，根据提示的修复建议进行操作。
- **改进** 快速自检命令
  - 环境检查：.\run_win.ps1 -Check（显示详细的组件状态）
  - 安装依赖：.\run_win.ps1 -InstallDeps（智能安装所有缺失依赖）
  - 构建：.\run_win.ps1 -Build
  - 运行：.\run_win.ps1 -Run
  - 测试：.\run_win.ps1 -Test

**更新** 故障排除指南现在包含更详细的错误诊断信息和智能修复建议。
...
## 结论
本项目以 Visual Studio 2022 + MSBuild 为核心构建系统，配合 vcpkg 与 NuGet 管理依赖，并通过增强的 PowerShell 脚本实现一键化构建与运行。**更新** 新的依赖管理系统提供了更智能的环境检测、自动化的依赖安装和详细的故障诊断信息，大大简化了开发环境的搭建过程。CI 流水线保证每次提交均可复现构建与测试。遵循本文的环境搭建、构建与发布流程，可稳定产出可分发的 Windows 桌面应用。
...
### 常用命令速查
- 环境检查：.\run_win.ps1 -Check（显示详细的组件状态和修复建议）
- 安装依赖：.\run_win.ps1 -InstallDeps（智能安装所有缺失依赖）
...
- **智能环境检测**：详细检查 vswhere、VS 工作负载、MSBuild、平台工具集、SDK、vcpkg、NuGet 等组件状态
- **自动依赖下载**：自动下载缺失的 nuget.exe 到用户目录
- **增量安装**：智能检测已安装的依赖，避免重复下载和构建
- **VS 集成支持**：优先使用 VS 内置的 vcpkg，支持独立安装的 vcpkg
- **详细错误报告**：提供清晰的错误信息和修复建议
- **跨平台兼容**：支持不同的 Windows 环境和 VS 安装路径

章节来源
- [run_win.ps1:32-86](file://run_win.ps1#L32-L86)
- [run_win.ps1:98-202](file://run_win.ps1#L98-L202)