...
## 更新摘要
**变更内容**   
- 新增环境检测功能 -Check 参数，提供全面的构建环境验证
- 新增依赖安装功能 -InstallDeps 参数，自动化 NuGet 和 vcpkg 依赖安装
- 更新快速开始命令流程，包含环境检查和依赖安装的完整工作流
- 增强错误提示和修复建议，提供更友好的用户体验
...
**新增功能**：现在提供了增强的环境检测和依赖安装功能，确保构建环境的完整性和一致性。
...
A --> F["run_win.ps1增强构建脚本"]
A --> G["run_win.bat批处理入口"]
...
### 环境与依赖准备
**新增功能**：现在提供了自动化的环境检测和依赖安装功能，大大简化了初始设置过程。

#### 前置要求
- 安装 Visual Studio 2022（需包含 C++ 工作负载）。
- 系统需要支持 PowerShell 执行策略。
...
**第二步：安装依赖**
```powershell
.\run_win.ps1 -InstallDeps
```
此命令会自动安装所有必需的依赖：
- 下载并配置 nuget.exe（如不存在）
- 还原 NuGet 包（gmock 等测试依赖）
- 安装 vcpkg 依赖（boost-asio、simpleini、opus）
- 使用 x64-windows-static-md triplet 进行静态链接
...
```powershell
# 一键式完整流程
.\run_win.ps1 -Check -InstallDeps -Build -Run
```
...
- [run_win.ps1:266-283](file://run_win.ps1#L266-L283)
- [run_win.bat:1-3](file://run_win.bat#L1-L3)
...
## 故障排除指南
**新增功能**：环境检测功能现在提供了更详细的错误诊断和修复指导。

### 环境检测问题
- **环境检测失败**：
  - 运行 `.\run_win.ps1 -Check` 查看具体问题
  - 根据输出的修复建议执行相应操作
  - 大多数情况下运行 `.\run_win.ps1 -InstallDeps` 即可解决
...
- **NuGet/vcpkg 不可用**：
  - 使用 `.\run_win.ps1 -InstallDeps` 自动下载安装所需工具。
  - 脚本会自动下载 nuget.exe 到用户目录。
- **端口冲突或服务端无法启动**：
...
## 结论
通过以上步骤，你可以在 Windows 上快速完成 SoundRemote-WinServer 的环境搭建、构建与运行，并与 Android 客户端建立稳定的音频传输链路。**新增的环境检测和依赖安装功能**使得初始设置过程更加简单可靠，即使遇到环境问题也能获得清晰的修复指导。建议在正式使用前根据实际网络与硬件条件调整压缩格式与端口配置，以获得最佳体验。
...
- **构建（Release|x64）**：
  - .\run_win.ps1 -Build
- **构建自定义配置/平台/输出目录**：
  - .\run_win.ps1 -Build -Configuration Debug -Platform Win32 -Output out
- **运行最新构建产物**：
  - .\run_win.ps1 -Run
- **通过批处理快捷入口运行**：
  - .\run_win.bat -Run
...
**章节来源**
- [run_win.ps1:1-24](file://run_win.ps1#L1-L24)
- [run_win.ps1:98-165](file://run_win.ps1#L98-L165)
- [run_win.ps1:167-202](file://run_win.ps1#L167-L202)
- [run_win.ps1:204-249](file://run_win.ps1#L204-L249)
- [run_win.ps1:266-283](file://run_win.ps1#L266-L283)
